plot_bericht <- function(filename, encoding = 'WINDOWS-1252', set_lwd = 50,
  ...
  #test if all species in "Baumart" are specified within the object baumarten
  if(any(!(df[ , 3] %in% baumarten[,1]))){
  ...
  #legend
  legende <- baumarten[unique(dat$Holzart),]
  legend('bottom', legend = legende[, 1], fill = as.character(legende[, 2]),
